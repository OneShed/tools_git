...
# Compare the repo definition set in jenkins jobs matches the settings in Amadeus.xml
...
for file in $(find -maxdepth 2 | grep config.xml | egrep -v _pipe | egrep -v perl | egrep -v emergency | egrep -v WEEKLY_MF_IMPORT | egrep -v '[[:lower:]]\/*\/' | egrep -v 'FORTIFY$'); do
        set +u
        set +e
